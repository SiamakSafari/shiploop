...
import { useConfetti, STREAK_MILESTONES, shouldCelebrateMilestone } from "@/hooks/use-confetti";
import { toast } from "sonner";

export function StreakCounter() {
  const user = useAppStore((state) => state.user);
  const [mounted, setMounted] = useState(false);
  const { fireworks, sideCannons } = useConfetti();
  ...
  useEffect(() => {
    ...
    const currentStreak = user.shipScore.streak.currentStreak;
    ...
    if (previousStreakRef.current !== null) {
      ...
      if (milestone) {
        ...
        toast.success(`${milestone}-Day Streak!`, {
          description: `You're on fire! ${milestone} days of consistent shipping!`,
        });
      }
    }

    previousStreakRef.current = currentStreak;
  }, [user?.shipScore.streak.currentStreak, fireworks, sideCannons]);
  ...
  return (
    <div className="glass hover-lift rounded-2xl relative overflow-hidden p-4">
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right rounded-xl bg-gray-50 dark:bg-gray-800/50 px-4 py-3 border border-border">
                ...
                <Micro
                  ...
                  className={cn(
                    ...
                    streak.currentStreak >= m
                      ...
                      : "bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 border border-border"
                  )}
                >
                  {m}
                </Micro>
