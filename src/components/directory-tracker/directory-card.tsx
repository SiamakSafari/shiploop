  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        {/* Icon */}
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm shrink-0 group-hover:scale-105 transition-transform">
          {(() => { const Icon = iconMap[directoryInfo.icon]; return Icon ? <Icon className="h-4 w-4 text-primary" /> : <span className="text-lg">{directoryInfo.icon}</span>; })()}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <Micro className="text-gray-500 dark:text-gray-400">
              {submission.projectName} · {completedCount}/{totalCount}
            </Micro>
          ...
          <div className="h-1 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden mt-1.5">
            <div
              className={cn(
                ...
                submission.status === "rejected" ? "bg-red-500" : "bg-primary"
              )}
